...
#define K_Cell @"cell"
@interface LSHGovernmentViewController ()
...
@implementation LSHGovernmentViewController
...
- (void)viewDidLoad {
    ...
    [_tableView registerClass:[LSHGovernmentViewCell class] forCellReuseIdentifier:K_Cell];
...
- (void)loadData
{
    __weak LSHGovernmentViewController *vc = self;
    [LSHNetworkingManager getWithURLString:[LSHNewUrl getNewsList] parameters:@{@"type":@"info"} success:^(NSDictionary *data) {
...
{
    LSHGovernmentViewCell *cell = [tableView dequeueReusableCellWithIdentifier:K_Cell forIndexPath:indexPath];
...
- (UIImageView *)image
{
    if (!_image) {
        ...
        _image.image = [UIImage imageNamed:@"zixun_mg_25"];
